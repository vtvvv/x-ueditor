...
public class BucketService {
	 ...
	 * @return boolean exists public static boolean doesBucketExist(OSSClient
	 *         client, String bucketName) { // 获取Bucket的存在信息 boolean exists =
	 *         client.doesBucketExist(bucketName); // 输出结果 if (exists) {
	 *         logger.debug("Bucket: " + bucketName + " is exists"); } else {
	 *         logger.debug("Bucket: " + bucketName + " is not exists"); } return
	 *         exists; }
	 */
	 ...
	 * @param bucketName bucketName
	 * @param acl<br>
	 *                   CannedAccessControlList是枚举类型，包含三个值： Private 、 PublicRead 、
	 *                   PublicReadWrite ，它们分别对应相关权限
	 */
